...
// Priority: 1) Vite ENV (Vercel/production), 2) /deployed/addresses.json (local dev), 3) hardcoded fallback
let addressCache = null;

async function loadAddresses() {
  if (addressCache) return addressCache;

  // Production: from Vite env (Vercel)
  const envFactory = import.meta.env?.VITE_FACTORY_ADDRESS;
  const envToken = import.meta.env?.VITE_TOKEN_ADDRESS;
  if (envFactory && envToken &&
      envFactory !== "0x0000000000000000000000000000000000000000") {
    addressCache = { FundFactory: envFactory, InvestToken: envToken };
    return addressCache;
  }

  // Local dev: from deploy artifact
  try {
